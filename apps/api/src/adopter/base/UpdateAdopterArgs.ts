/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AdopterWhereUniqueInput } from "./AdopterWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { AdopterUpdateInput } from "./AdopterUpdateInput";

@ArgsType()
class UpdateAdopterArgs {
  @ApiProperty({
    required: true,
    type: () => AdopterWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AdopterWhereUniqueInput)
  @Field(() => AdopterWhereUniqueInput, { nullable: false })
  where!: AdopterWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => AdopterUpdateInput,
  })
  @ValidateNested()
  @Type(() => AdopterUpdateInput)
  @Field(() => AdopterUpdateInput, { nullable: false })
  data!: AdopterUpdateInput;
}

export { UpdateAdopterArgs as UpdateAdopterArgs };
